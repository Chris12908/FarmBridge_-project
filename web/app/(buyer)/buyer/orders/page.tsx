...
const STATUS_TABS: { value: OrderStatus | 'all'; label: string }[] = [
  ...
  { value: OrderStatus.DISPATCHED, label: 'On the way' },
...
export default function BuyerOrdersPage() {
  ...
  return (
    <div className="px-4 py-5 max-w-2xl mx-auto">
      <h1 className="text-xl font-black text-slate-800 mb-5">My Orders</h1>
        ...
        <TabsContent value="all">
          {isLoading ? (
            ...
            <EmptyState
              icon="receipt_long"
              title="No orders yet"
              description="Complete a negotiation to place your first order."
              action={<Link href="/buyer/marketplace" className={buttonVariants()}>Browse Products</Link>}
            />
          ) : (
            <div className="flex flex-col gap-4">
              {orders.map((order) => (
                <OrderCard key={order.id} order={order} variant="buyer" />
              ))}
            </div>
          )}
        </TabsContent>
