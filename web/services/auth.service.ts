...
export const authService = {
  ...
  async changePassword(dto: { currentPassword: string; newPassword: string }): Promise<{ message: string }> {
    const { data } = await axiosClient.patch<{ message: string }>('/auth/change-password', dto);
    return data;
  },
};
